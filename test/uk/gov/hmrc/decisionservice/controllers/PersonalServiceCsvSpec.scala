...
class PersonalServiceCsvSpec extends UnitSpec with WithFakeApplication with DecisionControllerClusterCsvSpec {
  ...
  val PERSONAL_SERVICE_SCENARIO_0_v150 = s"/test-scenarios/${Versions.VERSION150_FINAL}/personal-service/scenario-0.csv"
  val PERSONAL_SERVICE_SCENARIO_1_v150 = s"/test-scenarios/${Versions.VERSION150_FINAL}/personal-service/scenario-1.csv"
  val PERSONAL_SERVICE_SCENARIOS_v150 = s"/test-scenarios/${Versions.VERSION150_FINAL}/personal-service/scenarios.csv"
  ...
  "POST /decide" should {
    ...
    s"return 200 and correct response with the expected decision for personal service scenario 0 for version ${Versions.VERSION150_FINAL}" in {
      createRequestSendVerifyDecision(PERSONAL_SERVICE_SCENARIO_0_v150, Versions.VERSION150_FINAL)
    }
    s"return 200 and correct response with the expected decision for personal service scenario 1 for version ${Versions.VERSION150_FINAL}" in {
      createRequestSendVerifyDecision(PERSONAL_SERVICE_SCENARIO_1_v150, Versions.VERSION150_FINAL)
    }
    s"return 200 and correct response with the expected decision for personal service scenarios for version ${Versions.VERSION150_FINAL}" in {
      createMultipleRequestsSendVerifyDecision(PERSONAL_SERVICE_SCENARIOS_v150, Versions.VERSION150_FINAL)
    }
  }
}
